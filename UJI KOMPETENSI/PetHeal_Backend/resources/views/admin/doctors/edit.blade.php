...
<div class="glass-card rounded-2xl overflow-hidden border border-slate-200/50">
    <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-0.5">Update doctor details</p>
        </div>
    </div>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="relative">
                            <div class="w-24 h-24 rounded-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden" id="photo-preview-container">
                                @if($doctor->photo_url)
                                    <img id="photo-preview" class="w-full h-full object-cover" src="{{ $doctor->photo_url }}?t={{ time() }}">
                                @elseif($doctor->photo)
                                    <img id="photo-preview" class="w-full h-full object-cover" src="{{ asset('storage/' . $doctor->photo) }}?t={{ time() }}">
                                @else
                                    <img id="photo-preview" class="w-full h-full object-cover hidden">
                                    <span class="material-symbols-outlined text-slate-400 text-3xl" id="photo-placeholder">person</span>
                                @endif
                            </div>
                        </div>
                        <div class="flex-1">
                            ...
                            <p class="text-[10px] text-slate-400 mt-1">JPG, PNG. Max 2MB. Leave empty to keep current photo.</p>
                            @if($doctor->photo || $doctor->photo_url)
                            <p class="text-[10px] text-emerald-600 mt-1">✓ Current photo stored</p>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="available_days[]" id="available_days" multiple required
                        class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary focus:border-transparent text-sm outline-none transition-all">
                        @php
                            $rawDays = is_array($doctor->available_days) ? $doctor->available_days : (json_decode($doctor->available_days, true) ?? []);
                            $selectedDays = array_map('strtolower', $rawDays);
                        @endphp
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $doctor->start_time) }}" required
                            class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary focus:border-transparent text-sm outline-none transition-all">
                    </div>
                    <div>
                        <label for="end_time" class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">End Time</label>
                        <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $doctor->end_time) }}" required
                            class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary focus:border-transparent text-sm outline-none transition-all">
                    </div>
                </div>
                ...
                <div>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ $doctor->is_active ? 'checked' : '' }}
