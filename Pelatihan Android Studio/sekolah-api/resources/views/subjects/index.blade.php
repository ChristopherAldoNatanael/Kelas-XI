...
<div class="container mx-auto px-6 py-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
        <div class="mb-4 sm:mb-0">
            ...
            <p class="text-gray-600 mt-1">Manage academic subjects and teacher assignments</p>
        </div>
        <a href="{{ route('web-subjects.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 text-white font-semibold rounded-lg transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl">
    ...
    <div class="bg-white shadow-2xl rounded-2xl overflow-hidden transform transition-all duration-300 hover:shadow-3xl">
        <!-- Card Header -->
        <div class="bg-gradient-to-r from-green-50 to-teal-100 px-6 py-4 border-b border-gray-200">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                <div class="mb-4 sm:mb-0">
                    ...
                    <p class="text-sm text-gray-600 mt-1">Total: {{ $subjects->total() }} subjects</p>
                </div>
                <div class="w-full sm:w-auto">
                    <div class="relative">
                        ...
                        <input type="text" id="searchInput" placeholder="Search subjects..."
                               class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 transition-colors duration-200">
                    </div>
                </div>
            </div>
        </div>
                    ...
                    @forelse($subjects as $subject)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 rounded-lg bg-gradient-to-r from-green-400 to-teal-500 flex items-center justify-center">
                                        <span class="text-white font-bold text-sm">{{ $subject->kode }}</span>
                                    </div>
                                    <div class="ml-3">
                                        <div class="text-sm font-medium text-gray-900">{{ $subject->kode }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $subject->nama }}</div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $subject->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('web-subjects.show', $subject->id) }}"
                                    ...
                                    <a href="{{ route('web-subjects.edit', $subject->id) }}"
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('web-subjects.restore', $subject->id) }}" class="inline">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        <form method="POST" action="{{ route('web-subjects.force-delete', $subject->id) }}" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                    class="text-red-600 hover:text-red-900 p-2 rounded-lg hover:bg-red-50 transition-colors duration-200"
                                                    title="Permanently Delete"
                                                    onclick="return confirm('Are you sure you want to permanently delete this subject?')">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <div class="mt-4">
                                        <a href="{{ route('web-subjects.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors duration-200">
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-700">
                        Showing <span class="font-medium">{{ $subjects->firstItem() }}</span> to <span class="font-medium">{{ $subjects->lastItem() }}</span> of <span class="font-medium">{{ $subjects->total() }}</span> results
                    </div>
