...
@section('title', 'Edit Subject - ' . $subject->nama)
...
<div class="min-h-screen">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-white/70 text-lg md:text-xl leading-relaxed max-w-2xl">
                            Update subject information and settings for <span class="text-emerald-300 font-semibold">{{ $subject->nama }}</span>.
                        </p>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-4 pt-4">
                        ...
                        <div class="glass-stat-card">
                            <div class="text-2xl font-bold text-white">{{ $subject->created_at->diffForHumans() }}</div>
    ...
    <div class="px-6">
        <form method="POST" action="{{ route('web-subjects.update', $subject->id) }}" class="max-w-4xl mx-auto space-y-8" id="subjectEditForm">
            @csrf
            @method('PUT')
            ...
            <div class="glass-morphism-card p-8">
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-slate-300">Update the basic information for this subject</p>
                    </div>
                </div>
            ...
            <div class="glass-morphism-card p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="category" name="category" class="glass-search-input @error('category') border-red-400 @enderror" required>
                            ...
                            <option value="wajib" {{ old('category', $subject->category) == 'wajib' ? 'selected' : '' }}>
                                📚 Required (Wajib) - Mandatory course
                            </option>
                            <option value="peminatan" {{ old('category', $subject->category) == 'peminatan' ? 'selected' : '' }}>
                                🎯 Specialization (Peminatan) - Elective course
                            </option>
                            <option value="mulok" {{ old('category', $subject->category) == 'mulok' ? 'selected' : '' }}>
                ...
                <div class="mt-6 space-y-2">
                    ...
                    <textarea id="description"
                              ...
                              placeholder="Enter subject description, learning objectives, and key topics...">{{ old('description', $subject->description) }}</textarea>
                    @error('description')
                        <div class="text-red-400 text-sm mt-1 flex items-center gap-1">
                            <i class="fas fa-exclamation-circle"></i>
                            {{ $message }}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="credit_hours" name="credit_hours" class="glass-search-input">
                            @for($i = 1; $i <= 6; $i++)
                                <option value="{{ $i }}" {{ old('credit_hours', $subject->credit_hours) == $i ? 'selected' : '' }}>
                                    {{ $i }} Credit{{ $i > 1 ? 's' : '' }}
                                </option>
                            @endfor
                        </select>
            ...
            <div class="glass-morphism-card p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="glass-stat-mini">
                        <div class="text-lg font-bold text-white">#{{ $subject->id }}</div>
                        <div class="text-xs text-slate-400">Record ID</div>
                    </div>
                    <div class="glass-stat-mini">
                        <div class="text-lg font-bold text-white">{{ $subject->created_at->format('M d, Y') }}</div>
                        <div class="text-xs text-slate-400">Created</div>
                    </div>
                    <div class="glass-stat-mini">
                        <div class="text-lg font-bold text-white">{{ $subject->updated_at->format('M d, Y') }}</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Updating Subject...';

            // Re-enable after 10 seconds if still on page
            setTimeout(() => {
                if (submitBtn.disabled) {
                    submitBtn.disabled = false;
                    submitBtn.innerHTML = '<i class="fas fa-save mr-2"></i>Update Subject';
                }
            }, 10000);
        });
    }
